...
function ForgotPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const data = await apiPost('/api/forget-password', { 
        email: trimmedEmail 
      });
    ...
    <div style={{
      ...
      {window.innerWidth <= 768 ? (
        ...
        <div style={{
          ...
          <div style={{ fontSize: 22, fontWeight: 800, color: '#1e40af', marginBottom: 12, textAlign: 'center' }}>Forgot Password</div>
          ...
          <form onSubmit={handleSubmit} style={{ width: '100%', maxWidth: 300 }}>
            ...
            {message && <div style={{ color: 'green', fontSize: 14, marginBottom: 12 }}>{message}</div>}
            <button 
              type="submit" 
              disabled={loading} 
              style={{ 
          ...
          <div style={{ marginTop: 8 }}>
            <span style={{ fontSize: 14, color: '#64748b' }}>Remembered your password? <Link to="/" style={{ color: '#3b82f6', fontWeight: 600, textDecoration: 'none' }}>Login</Link></span>
          </div>
        </div>
      ) : (
        // Desktop Layout
