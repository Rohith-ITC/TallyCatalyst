// Approved Payments Page - View all approved/paid payments (Superadmin only)
import React, { useState, useEffect } from 'react';
import { getApprovedPayments } from '../api/subscriptionApi';
...
const ApprovedPaymentsPage = () => {
  ...
  const fetchApprovedPayments = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await getApprovedPayments(filters);
  ...
  if (loading && approvedPayments.length === 0) {
    return (
  ...
  return (
    <div className="payment-validation-page">
      <div className="validation-page-header">
        ...
        <p>View all approved and paid payment submissions</p>
      </div>
      ...
      {error && (
        <div className="error-container">
          ...
          <button onClick={fetchApprovedPayments} className="retry-button">
            Retry
          </button>
        </div>
      )}

      {approvedPayments.length === 0 && !loading ? (
